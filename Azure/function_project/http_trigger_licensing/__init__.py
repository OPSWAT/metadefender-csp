...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        data = req.get_json()
        logging.info("Payload Recived: %s", data)
    except ValueError:
        return func.HttpResponse("Invalid JSON", status_code=400)

    try:
        condition = data.get("data", {}).get("alertContext", {}).get("condition", {}).get("allOf", [])[0]
        logging.info("condition: %s", condition)
        rg_name = data.get("data", {}).get("essentials", {}).get("targetResourceGroup")
        logging.info("rg_name: %s", rg_name)
        metric_value = condition.get("metricValue")
        logging.info("metric_value: %s", metric_value)
        vm_name = next((d.get("value") for d in condition.get("dimensions", []) if d["name"] == "VMName"), "unknown")
        logging.info("vm_name: %s", vm_name)
    except Exception as e:
        logging.error(f"Malformed alert: {e}")
        return func.HttpResponse("Ignored: Incomplete data", status_code=502)

    if metric_value is None or vm_name == "unknown":
        return func.HttpResponse("Missing metric or VM name", status_code=502)
    ...
    logging.info("coreUser: %s", core_user)
    core_pwd = secrets["corePwd"]
    logging.info("corePwd: %s", core_pwd)
    license_key = secrets["licenseKey"]
    logging.info("licenseKey: %s", license_key)
    apikey = secrets["apiKey"]
    logging.info("apiKey: %s", apikey)

    try:
        if float(metric_value) < 1:
            ...
            if (apikey == ""):
                ...
                logging.info("apikey/session ID: %s", apikey)
